#!/usr/bin/env luajit
-- Test that Space toggles a focused checkbox
...
if not sdl.SDL_CreateWindowAndRenderer("Checkbox Space Test", 300, 200, 0, window, renderer) then
...
-- Focus the first widget (checkbox)
ctx.id = 1

-- Frame baseline
gui.gui_begin(renderer[0], texture, ctx)
local changed = gui.gui_checkbox(ctx, 50, 50, "Check me", checked)
gui.gui_end(ctx)
assert_true(not changed and (not checked[0]), "Baseline: unchecked and not changed")

-- Press Space
key_down(sdlm.SDLK_SPACE)

gui.gui_begin(renderer[0], texture, ctx)
changed = gui.gui_checkbox(ctx, 50, 50, "Check me", checked)
gui.gui_end(ctx)
assert_true(changed and checked[0], "Space should toggle checkbox on")
...
changed = gui.gui_checkbox(ctx, 50, 50, "Check me", checked)
gui.gui_end(ctx)
assert_true(changed and (not checked[0]), "Space should toggle checkbox off")
...
print("[PASS] Space toggles focused checkbox")
